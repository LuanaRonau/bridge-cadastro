...
Fluxo:
  ...
  2. Busca (Search API) issues fechadas desde a última execução, em todos os
     repositórios configurados da organização.
...
def find_project_id(session, org, project_title):
    ...
    cursor = None
    while True:
        data = graphql(session, query, {"org": org, "cursor": cursor})
        proj = data["organization"]["projectsV2"]
...
def get_ladybug_issue_set(session, org, project_title):
    """Retorna um set de (owner, repo, number) para todas as issues do board Ladybug."""
    project_id = find_project_id(session, org, project_title)
    if not project_id:
        print(f"[aviso] Project '{project_title}' não encontrado na org '{org}'.")
        return set()
    ...
    while True:
        ...
        for node in items["nodes"]:
            ...
            result.add((owner.lower(), repo.lower(), content["number"]))
...
def search_recently_closed_issues(session, org, repos, since_dt):
    """Usa a Search API para achar issues fechadas desde `since_dt` nos repos configurados."""
    since_date = since_dt.strftime("%Y-%m-%d")
    repo_filters = " ".join(f"repo:{org}/{r}" for r in repos)
...
def main():
    ...
    ladybug_set = get_ladybug_issue_set(session, org, ladybug_title)
    print(f"Board Ladybug: {len(ladybug_set)} issue(s) mapeada(s).")

    closed_issues = search_recently_closed_issues(session, org, repos, since_dt)
    ...
    for issue, closed_dt in candidates:
        ...
        owner, repo = repo_url.split("/repos/")[1].split("/")
        milestone_number = issue["milestone"]["number"]
        milestone_title = issue["milestone"]["title"]
        gkey = (owner, repo, milestone_number)
        groups.setdefault(gkey, {"title": milestone_title, "issues": []})
        groups[gkey]["issues"].append((issue, closed_dt))

    for (owner, repo, milestone_number), info in groups.items():
        milestone_title = info["title"]
        open_now = get_open_issues_in_milestone(session, owner, repo, milestone_number)
        ...
        non_ladybug_batch = [
            ...
            if (owner.lower(), repo.lower(), issue["number"]) not in ladybug_set
        ]
        ladybug_batch = [
            (issue, dt) for issue, dt in batch
